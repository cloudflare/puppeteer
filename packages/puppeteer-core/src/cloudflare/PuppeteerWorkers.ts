...
export class PuppeteerWorkers extends Puppeteer {
  ...
  public async launch(
    ...
    if (options?.outboundByHost && (options.browser || typeof endpoint.launch !== 'function')) {
      throw new Error('outboundByHost requires a Browser Run RPC binding');
    }
    if (wantsRpcLaunch && !options?.browser && typeof endpoint.launch === 'function') {
  ...
  public override async connect(
    ...
    if (options?.outboundByHost) {
      ...
      if (sessionId || options.browser || typeof browserWorker.launch !== 'function') {
        throw new Error('outboundByHost must be passed to launch() or acquire() on a Browser Run RPC binding');
      }
    ...
    try {
      ...
      if (sessionId && rpcBindings.has(connectionEndpoint) && !sessionPinnedEndpoints.has(connectionEndpoint)) {
  ...
  public async acquire(
    ...
    validateKitesurfOptions(options);
    if (options?.outboundByHost && (options.browser || typeof endpoint.acquire !== 'function')) {
      throw new Error('outboundByHost requires a Browser Run RPC binding');
    }
